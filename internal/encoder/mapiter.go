...
func (self *_MapIterator) appendGeneric(p *_MapPair, t *rt.GoType, v reflect.Kind, k unsafe.Pointer) error {
	switch v {
	case reflect.Int:
		p.k = rt.Mem2Str(p.m[:native.I64toa(&p.m[0], int64(*(*int)(k)))])
		return nil
	case reflect.Int8:
		p.k = rt.Mem2Str(p.m[:native.I64toa(&p.m[0], int64(*(*int8)(k)))])
		return nil
	case reflect.Int16:
		p.k = rt.Mem2Str(p.m[:native.I64toa(&p.m[0], int64(*(*int16)(k)))])
		return nil
	case reflect.Int32:
		p.k = rt.Mem2Str(p.m[:native.I64toa(&p.m[0], int64(*(*int32)(k)))])
		return nil
	case reflect.Int64:
		p.k = rt.Mem2Str(p.m[:native.I64toa(&p.m[0], *(*int64)(k))])
		return nil
	case reflect.Uint:
		p.k = rt.Mem2Str(p.m[:native.U64toa(&p.m[0], uint64(*(*uint)(k)))])
		return nil
	case reflect.Uint8:
		p.k = rt.Mem2Str(p.m[:native.U64toa(&p.m[0], uint64(*(*uint8)(k)))])
		return nil
	case reflect.Uint16:
		p.k = rt.Mem2Str(p.m[:native.U64toa(&p.m[0], uint64(*(*uint16)(k)))])
		return nil
	case reflect.Uint32:
		p.k = rt.Mem2Str(p.m[:native.U64toa(&p.m[0], uint64(*(*uint32)(k)))])
		return nil
	case reflect.Uint64:
		p.k = rt.Mem2Str(p.m[:native.U64toa(&p.m[0], *(*uint64)(k))])
		return nil
	case reflect.Uintptr:
		p.k = rt.Mem2Str(p.m[:native.U64toa(&p.m[0], uint64(*(*uintptr)(k)))])
		return nil
